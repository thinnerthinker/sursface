...
use sursface::wgpu::{BindGroup, Buffer, BufferAddress, BufferUsages, Color, CommandEncoderDescriptor, PipelineLayoutDescriptor, RenderPipeline, VertexAttribute, VertexBufferLayout, VertexFormat, VertexStepMode};
    ...
    fn render(&mut self, display: &mut Display) {
        ...
        let output = {
    ...
    
            {
                ...
                self.uniforms.cursor_pos = [self.cursor_location.x / display.config.width as f32, self.cursor_location.y / display.config.height as f32];
    ...
    fn event<'a>(&mut self, display: &mut Display, event: WindowEvent) {
        match event {
            WindowEvent::CursorMoved { position, .. } => {
                self.cursor_location = PhysicalPosition { x: position.x as f32, y: position.y as f32 };
                
                self.interaction_state = match self.interaction_state.clone() {
                    InteractionState::PanningIdle { pressed_down_at: _, pre_tap: _ } | InteractionState::Zooming | InteractionState::Panning => {
                        let dx = (self.cursor_location.x - self.last_cursor_location.x) / display.size.width as f32;
                        let dy = (self.cursor_location.y - self.last_cursor_location.y) / display.size.height as f32;
            ...
            WindowEvent::MouseInput { state: elem_state, button, .. } => {
                self.interaction_state = if elem_state == ElementState::Pressed && button == MouseButton::Left {
                    self.last_cursor_location = self.cursor_location;
                ...
                } else { self.interaction_state.clone() };
